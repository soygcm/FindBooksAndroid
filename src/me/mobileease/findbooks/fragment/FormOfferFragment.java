...
public class FormOfferFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (form == 0) {
			...
			rootView = inflater.inflate(R.layout.fragment_sell, container,
					false);
			...
			rootView = inflater.inflate(R.layout.fragment_gift, container,
					false);
		}
	 ...
	 * getResources().getStringArray(R.array.currency_codes_array);
	 * 
	 * List<String> currencyCodeList = Arrays.asList(currencyCodeArray);
	 * 
	 * for ( CurrencyCode code : CurrencyCode.values() ) {
	 * 
	 * Currency currency = code.getCurrency(); Iterator<CountryCode> i =
	 * code.getCountryList().iterator();
	 * 
	 ...
	 * 
	 * currencyNames.add(code.getName() +" ("+ currency.getSymbol(
	 * country.toLocale() ) + ") " ); }
	 * 
	 * }
	 * 
	 ...
	 * 
	 * try { Currency currency = Currency.getInstance(code); currencyNames.add(
	 * currency.getSymbol() ); } catch (IllegalArgumentException e) {
	 * 
	 ...
	 * R.array.currency_codes_array, android.R.layout.simple_spinner_item); //
	 * Specify the layout to use when the list of choices appears
	 * adapter.setDropDownViewResource
	 * (android.R.layout.simple_spinner_dropdown_item); // Apply the adapter to
	 * the spinner spinner.setAdapter(adapter);
	 */
}
